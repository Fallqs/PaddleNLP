...
from tio_dataset import create_pretrained_dataset
from args import parse_args
import lr

sys.path.insert(0, "../")
from data_tools.tio_gen2 import Sampler
...
def generate(texts, model, tokenizer, args):
    ...
    for i, text in enumerate(texts):
        ...
        model = model._layers if isinstance(model,
        ...
        print(
            f'input text: {text}, generated_ids: {generated_ids}, generated text: \n{(text + generated_text).strip()}'
        )
...
def run_evaluate(data_loader,
    ...
    for eval_step, batch in enumerate(data_loader):
        ...
        if eval_step >= iter_steps - 1:
            break
    if test_texts:
        generate(test_texts, model, tokenizer, args)
...
def get_train_data_file(args):
    files = [
        ...
        if (os.path.isfile(os.path.join(args.input_dir, f))
            and str(f).endswith("_idx_tio.npz"))
    ]
            ...
            for step, batch in enumerate(train_data_loader()):
                ...
                if global_step >= args.max_steps:
                    run_evaluate(test_data_loader, model, criterion,
                                 ...
                                 epoch, tokenizer, None, None, "test")
